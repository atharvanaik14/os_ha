...
get_os() {
    ...
    case $OSTYPE in
        ...
        haiku)
            ...
            [[ $FM_TRASH_CMD == trash ]] && {
                FM_TRASH=$(finddir -v "$PWD" B_TRASH_DIRECTORY)
                mkdir -p "$FM_TRASH"
            }
        ;;
    esac
...
setup_options() {
    ...
    [[ $FM_FILE_FORMAT == *%f* ]] && {
        file_pre=${FM_FILE_FORMAT/'%f'*}
        file_post=${FM_FILE_FORMAT/*'%f'}
    }

    # Format for marked files.
    # Use affixes provided by the user or use defaults, if necessary.
    if [[ $FM_MARK_FORMAT == *%f* ]]; then
        mark_pre=${FM_MARK_FORMAT/'%f'*}
        mark_post=${FM_MARK_FORMAT/*'%f'}
    else
        mark_pre=" "
        mark_post="*"
    fi
...
get_w3m_path() {
    ...
    read -r w3m _ < <(type -p "$FM_W3M_PATH" w3mimgdisplay "${w3m_paths[@]}")
}
...
mark() {
    ...
    case "$2" in
        ${FM_KEY_YANK:=y}|${FM_KEY_YANK_ALL:=Y}) file_program=(cp -iR) ;;
        ${FM_KEY_MOVE:=m}|${FM_KEY_MOVE_ALL:=M}) file_program=(mv -i)  ;;
        ${FM_KEY_LINK:=s}|${FM_KEY_LINK_ALL:=S}) file_program=(ln -s)  ;;

        # These are 'FM' functions.
        ${FM_KEY_TRASH:=d}|${FM_KEY_TRASH_ALL:=D})
            file_program=(trash)
        ;;

        ${FM_KEY_BULK_RENAME:=b}|${FM_KEY_BULK_RENAME_ALL:=B})
...
trash() {
    ...
    if [[ $FM_TRASH_CMD ]]; then
        ...
        command "$FM_TRASH_CMD" "${@:1:$#-1}"

    else
        cd "$FM_TRASH" || cmd_line "error: Can't cd to trash directory."
...
bulk_rename() {
    ...
    rename_file=${XDG_CACHE_HOME:=${HOME}/.cache}/FM/bulk_rename
    marked_files=("${@:1:$#-1}")
...
open() {
    ...
    if [[ -d $1/ ]]; then
        ...
        case "$mime_type" in
            text/*|*x-empty*|*json*)
                # If 'FM' was opened as a file picker, save the opened
                # file in a file called 'opened_file'.
                ((file_picker == 1)) && {
                    printf '%s\n' "$1" > \
                        "${XDG_CACHE_HOME:=${HOME}/.cache}/FM/opened_file"
                    exit
                }
            ...
            *)
                ...
                nohup "${FM_OPENER:-${opener:-xdg-open}}" "$1" &>/dev/null &
                disown
            ;;
        esac
    fi
...
key() {
    ...
    case ${special_key:-$1} in
        ...
        # ('\e[B' or '\eOB').
        ${FM_KEY_SCROLL_DOWN1:=j}|\
        ${FM_KEY_SCROLL_DOWN2:=$'\e[B'}|\
        ${FM_KEY_SCROLL_DOWN3:=$'\eOB'})
            ((scroll < list_total)) && {
                ((scroll++))
                ((y < max_items)) && ((y++))
        ...
        # ('\e[A' or '\eOA').
        ${FM_KEY_SCROLL_UP1:=k}|\
        ${FM_KEY_SCROLL_UP2:=$'\e[A'}|\
        ${FM_KEY_SCROLL_UP3:=$'\eOA'})
        ...
        ${FM_KEY_HIDDEN:=.})
            ...
            #                some modification.
            shopt_flags=(u s)
            shopt -"${shopt_flags[((a=${a:=$FM_HIDDEN}>0?0:++a))]}" dotglob
            redraw full
        ;;
        ...
        ${FM_KEY_SHELL:=!})
            ...
            # Make FM aware of how many times it is nested.
            export FM_LEVEL
            ((FM_LEVEL++))
        ...
        q)
            : "${FM_CD_FILE:=${XDG_CACHE_HOME:=${HOME}/.cache}/FM/.FM_d}"
...
main() {
    ...
    # Store file name in a file on open instead of using 'FM_OPENER'.
    # Used in 'FM.vim'.
    [[ $1 == -p ]] &&
        file_picker=1
    ...
    # SEE: https://github.com/dylanaraps/FM/issues/48
    ((BASH_VERSINFO[0] > 3)) &&
        read_flags=(-t 0.05)
    ...
    mkdir -p "${XDG_CACHE_HOME:=${HOME}/.cache}/FM" \
             "${FM_TRASH:=${XDG_DATA_HOME:=${HOME}/.local/share}/FM/trash}"
